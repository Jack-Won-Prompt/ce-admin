...
<style>
  ...
  /* 프레임 안에서는 화면 단추 줄을 두지 않는다 (2026-09-16 지시).

     네비바가 숨는 탭 안에서도 단추를 쓸 수 있게 같은 것을 한 벌 더 두는 자리인데
     (.framed-actions), 이 화면이 내놓는 것은 상태 배지와 「목록」ㆍ「홈」 뿐이다.
     탭 워크스페이스에서는 셋 다 쓸모가 없다 — 상태는 환자 정보바가 이미 적고,
     목록과 홈은 탭이 하는 일이며, 오히려 보던 주문에서 지금 탭을 떠나 버린다.

     그러면서 한 줄을 통째로 먹어, 화면 맨 위에 서야 할 환자 정보바가 화면탭 아래로
     한 칸 더 내려갔다. 이 화면에서만 그 줄을 내린다 — 다른 화면의 「신규 접수」
     같은 단추는 프레임 안에서 이 줄이 유일한 자리라 그대로 둔다.

     지우지 않고 감추기만 한다. 배지(#rxStatusBadge)를 찾아 고쳐 세우는 자리가 있어
     문서에는 남아 있어야 한다(네비바 쪽이 문서 순서상 먼저라 그쪽이 잡힌다). */
  html.is-framed .framed-actions { display: none !important; }

  /* 프레임 안에서는 정보바를 띄우지 않는다 (2026-09-16 고침).

     띄워 두면 top:0 에 서서 그 자리에 있던 화면 단추 줄을 덮었다. 단추가 보이지도
     눌리지도 않았고, 자리표시자가 비울 높이를 재는 셈도 어긋나 화면이 통째로 밀렸다.
        ...
        <div id="consentPopover" style="display:none;position:absolute;top:calc(100% + 8px);left:0;width:380px;background:var(--bg-card);border:1px solid var(--primary);border-radius:var(--radius-lg);box-shadow:0 8px 32px rgba(0,0,0,.18);z-index:502;">
          ...
          <div style="padding:14px;display:flex;flex-direction:column;gap:10px;">
            ...
            <div>
              ...
              <input data-phone type="text" class="form-control" id="consentMobile"
      ...
      <div style="position:relative;">
        ...
        <div id="idCardPopover" style="display:none;position:absolute;top:calc(100% + 8px);left:0;width:380px;background:var(--bg-card);border:1px solid var(--primary);border-radius:var(--radius-lg);box-shadow:0 8px 32px rgba(0,0,0,.18);z-index:502;">
          ...
          <div style="padding:14px;display:flex;flex-direction:column;gap:10px;">
            ...
            <div>
              <label style="font-size:11px;font-weight:500;color:var(--text-secondary);margin-bottom:4px;display:block;">수신 번호</label>
              <input data-phone type="text" class="form-control" id="idCardMobile"
                     placeholder="010-XXXX-XXXX / 02-XXXX-XXXX"
                     value="{{ $prescription->patient?->mobile ?? $prescription->mobile_ocr ?? '' }}"
          ...
          <div style="padding:14px;display:grid;grid-template-columns:1fr 1fr;gap:14px;align-items:stretch;">
            ...
            <div style="display:flex;flex-direction:column;gap:10px;">
              ...
              <div>
                ...
                <input data-phone type="text" id="fax-no" class="form-control" style="font-size:12px;height:32px;"
  ...
  <div class="order-layout">
    ...
    <div id="viewerInner">
      ...
      <div class="card mt-3" id="reviewMemoCard">
        <div class="card-header"><i class="fa-solid fa-note-sticky" style="color:var(--warning);"></i><span class="card-header-title">참고 사항</span></div>
      ...
      <div class="cv">
        ...
        <div class="rx-acc-item rx-tabbed">
          ...
          <div class="rx-acc-body rx-pane" id="rxp-1">
            ...
            {{-- 차례는 요청서 9쪽 그대로다. 보호자 관련 칸은 「미성년」 배지가 여는
                 팝오버에 있어 여기 서지 않는다. --}}
              {{-- 연락이 닿는가 — 사망ㆍ수신거부ㆍ타사이동은 다음에 할 일이 서로 다르다.
                   고치는 자리는 거래처관리다(요청서 1·9쪽). --}}
              <div class="rx-field-row rx-row-start">
                <span class="rx-field-label">연락 상태</span>
                <input type="text" class="form-control" id="f-contact-status" readonly
                       value="{{ $prescription->patient?->contactStatusLabel() }}" style="flex:1;" />
              </div>
              ...
              <div class="rx-field-row">
                <span class="rx-field-label">연락 선호 방식</span>
                <input type="text" class="form-control" id="f-contact-channel" readonly
                       value="{{ $prescription->patient?->contactChannelLabel() }}" style="flex:1;" />
              </div>
              ...
              <div class="rx-field-row">
                ...
                <input data-phone type="text" class="form-control" id="f-fax" readonly
              ...
              <div class="rx-field-row rx-row-start rx-w4" style="align-items:flex-start;">
                ...
                <div style="display:flex;flex-direction:column;gap:8px;flex:1;min-width:0;">
                  ...
                  <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;row-gap:8px;">
                    ...
                    {{-- 「주소 검색」은 걷었다(2026-09-09).

                         셋이 어긋났다. 요청서 9쪽은 상세 목록의 환자 정보를 「거래처
                         관리에서만 update 하고 연결만」 하라고 했는데, 주소 검색은 그
                         자리에서 새 값을 만드는 길이었다. 게다가 그렇게 찍어 넣은 주소는
                         이 건에만 남고 거래처의 주소 목록에는 들어가지 않아, 같은 사람에게
                         또 보낼 때 다시 찾아야 했다. 나란히 선 두 단추는 매번 어느 쪽을
                         눌러야 하는지 묻게 했다.
              ...
              <div class="rx-field-row">
                ...
                {{-- 두 칸이 폭을 반씩 나눠 가지면 번호가 잘린다.
                     ...
                     「소득공제 선택」이 들어갈 124px 를 준다(글자 71 + 패딩 42 = 113).
        ...
        ───────────────────────────────────────────────── --}}
          ...
          <div class="rx-acc-body rx-pane" id="rxp-2" style="display:none;">
            ...
            {{-- 요청서 13쪽의 차례 그대로 가로로 읽는다. 세 기둥(.rx-col)에
                 ...
                 한 줄로 늘어놓은 요청서의 차례와 맞출 수가 없었다.
                 상담ㆍ환자 정보가 이미 이 얼개다(.rx-rows · 6칸 격자). --}}
            <div class="rx-rows">
              ...
              <div class="rx-field-row rx-w4" style="position:relative;">
                <span class="rx-field-label">요류역학검사 결과</span>
              ...
              <div class="rx-field-row">
                <span class="rx-field-label">처방전종료일</span>
        ...
        <div class="card">
          <div class="card-body">
            ...
            {{-- 결제 걸음 (2026-09-25 지시).
                 ...
                 값은 이미 있는 것을 그대로 읽는다 — 새로 세는 자리를 만들지 않는다.
                 변수를 두지 않고 그때그때 묻는다(이 파일의 다른 걸음들과 같은 꼴). --}}
            <div class="workflow-step">
              <div class="ws-icon {{ $prescription->order && $prescription->order->isDepositConfirmed() ? 'done' : 'pending' }}"><i class="fa-solid fa-won-sign"></i></div>
              <div>
                <div class="ws-label">결제 완료</div>
                <div class="ws-time">
                  @if($prescription->order && $prescription->order->isDepositConfirmed())
                    {{ $prescription->order->paidAtLabel('H:i') }} · {{ $prescription->order->payMethodLabel() }}
                    <span style="color:var(--primary);font-weight:700;display:block;">&#8361;{{ number_format($prescription->order->받은금액()) }}</span>
                  @else
                    대기 중
                  @endif
                </div>
            ...
            <table class="tab-tbl">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    @if($prescription->order && $prescription->order->isDepositConfirmed())
                      {{ $prescription->order->paidAtLabel() }} · {{ $prescription->order->payMethodLabel() }}
                      <span style="color:var(--primary);font-weight:700;margin-left:6px;">&#8361;{{ number_format($prescription->order->받은금액()) }}</span>
                    @else대기 중@endif
                  </td>
                </tr>
                @if($prescription->order && $prescription->order->status === 'cancelled')
  ...
  <div class="hp-box">
    ...
    <div class="hp-new" id="hpNew" style="display:none;">
      ...
      <div class="hp-new-grid">
        ...
        <label>전화번호</label>
        <input data-phone type="text" id="hpTel" class="form-control" placeholder="02-0000-0000">
        <label>주소</label>
...
<script>
  ...
  document.getElementById('f-deduction')?.addEventListener('change', function () {
    ...
    if (this.value === '자진발급') {
      번호 = '{{ \App\Models\Patient::SELF_ISSUE_NO }}';
    } else if (this.value === '소득공제') {
      /* 전화번호 칸은 화면 설정에 따라 고르는 칸일 수도, 적는 칸일 수도 있다 */
      번호 = [...document.querySelectorAll('#f-mobile')]
...
지금 저장해 거래처로 올리고 상담을 열까요?`,
  ...
  /* 줄여 쓴 시도명 → 온전한 이름. 주소 검색(카카오)이 「서울 중구 세종대로 110」처럼
     시도를 줄여 준다. 아래 규칙은 「서울특별시」같은 온전한 이름만 받으므로, 펴 주지
     않으면 관할 지자체가 빈 채로 남는다 — 기초는 지자체에 등기로 청구하므로 그러면
     어디로 부칠지 모른다. 서버의 ClaimAgency 와 같은 표다. */
  const 시도줄임 = {
  ...
  function localGovFromAddress(address) {
    let addr = (address || '').replace(/\s+/g, ' ').trim();

    const 머리 = addr.split(' ')[0];
    if (시도줄임[머리]) addr = 시도줄임[머리] + addr.slice(머리.length);
  ...
  document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.rx-tabs .rx-acc-btns').forEach(묶음 => {
      묶음.addEventListener('click', (e) => {
        ...
        /* 줄바꿈은 배열로 이어 붙인다 — 이 파일이 지나는 길에서 이스케이프가
           실제 줄바꿈이 되어 문자열이 거기서 끊긴다 (이 저장소에서 세 번 겪었다). */
        ceAlert([
          '거래처를 먼저 선택해 주십시오.',
          '',
  ...
  /* ── 신규 등록: 처방전 없이 새 건을 시작한다 ────────────────────

     예전에는 현재 처방전 위에서 화면 입력만 비웠다. 그러면 저장할 때 현재 건이
  ...
  async function 동의확인(opts) {
    ...
    _동의알림함 = true;
    /* 문장 사이를 띄운다 (2026-09-26 시험에서 찾음). 여태 「…진행해 주십시오.아직
       받지 못한 동의」로 붙어 나와 어디서 끊기는지 알 수 없었다.
       줄바꿈은 역슬래시로 넣지 않는다 — 이 파일에서 실제 줄바꿈이 되어 문자열이
       거기서 끊긴 적이 있다. */
    const _줄 = String.fromCharCode(10);
    await ceAlert(
      '동의를 확인한 뒤 구매를 진행해 주십시오.' + _줄 + _줄
      + '아직 받지 못한 동의' + _줄 + 남은.map(t => '· ' + t).join(_줄) + _줄 + _줄
  ...
  /* 검수 단추를 세우는 일은 두 곳이 한다 — 상태(요청ㆍ승인)와 유형(처방외).
     상태 쪽이 세 갈래에서 저마다 return 하므로, 한 겹 감싸서 **끝난 뒤에** 유형을
     본다. 그러지 않으면 요청 상태가 승인 단추를 되살려 처방외에서도 열린다. */
  function 입력검수세우기(상태) {
    입력검수세우기_상태만(상태);
    window.검수단추가리기?.();
  }

  function 입력검수세우기_상태만(상태) {
    if (!상태) return;
    INPUT_REVIEW = 상태;
  ...
  function gateTotalCount(block) {
    ...
    /* 셈한 값을 먼저 보이고 할 일을 뒤에 적는다 (2026-09-26 시험에서 찾음).
       여태 두 문장을 그대로 이어 붙여 「…저장해 주십시오.1일 처방 개수: 6 …」으로
       한 덩어리로 붙어 나왔다 — 읽는 사람이 문장이 끊긴 자리를 못 찾는다. */
    ceAlert(msg + String.fromCharCode(10, 10)
  ...
  function gate청구한도(block) {
    ...
    /* 하루 한도가 이미 걸렸어도 수량은 따로 본다 (2026-09-16 고침).

       여태 `지날수있나 &&` 로 묶어 두어, 둘이 함께 넘으면 하루 한도만 알리고
       수량 초과는 묻혔다. 담당자는 한도를 고쳐 다시 누른 뒤에야 수량도 넘었다는
       것을 알았다 — 한 번에 알면 될 일을 두 번 오갔다. 막는 것은 어느 하나만
       걸려도 막고, 알리는 것은 걸린 것을 모두 알린다. */
    if (총계 > 0 && 합수량 > 총계) {
      const 말 = 다른주문 > 0
  ...
  function gateBillingStrategy() {
    ...
    if (!type) {
      ceAlert('유형이 비어 있어 주문을 생성할 수 없습니다.' + String.fromCharCode(10, 10)
    ...
    if (!key || !BILLING_STRATEGY[key]) {
      ceAlert('청구전략이 정해지지 않아 주문을 생성할 수 없습니다.' + String.fromCharCode(10, 10)
  ...
  async function createOrder(e) {
    ...
    /* 줄바꿈은 배열로 이어 붙인다 — 문자열 안에 그대로 두면 이 파일이 지나는
       길에서 실제 줄바꿈이 되어 문자열이 거기서 끊긴다(2026-09-15 겪음). */
    const 줄이음 = 줄들 => 줄들.join(String.fromCharCode(10));
    ...
    const 진행 = await 확인하고한번만({
      ...
      물음: 줄이음([
        '주문을 저장하고 위드웍스로 보냅니다.',
        '',
        `제품 ${담긴수.toLocaleString()}개가 창고로 전달되며, 전달 뒤에는 「주문 정정」으로만 수정할 수 있습니다.`,
        '',
        '진행하시겠습니까?',
      ]),
      확인글: '생성합니다',
  ...
  async function openCancelOrder(e) {
    ...
    /* 증빙은 이 자리에서 취소하지 않는다 (2026-09-16 확인).

       세금계산서ㆍ현금영수증은 국세청 실신고라, 주문을 무르는 김에 함께 부르지
       않는다. 반품ㆍ교환으로 닫는 길(OrderCancellation)은 자동으로 취소하지만,
       담당자가 손으로 누르는 이 자리는 「세금계산서 취소」ㆍ「현금영수증 취소」
       단추로 따로 확인하고 누르게 두었다.

       그 사실을 안내에 분명히 적는다 — 자동으로 되는 줄 알면 취소된 주문의
       계산서가 국세청에 그대로 남는다. */
    const 돈 = ['', '받은 결제가 있으면 함께 취소하고, 보낸 결제 링크는 해지합니다.',
                '세금계산서ㆍ현금영수증은 자동으로 취소되지 않습니다 — 발행된 건은 '
                  + '화면 위의 「세금계산서」ㆍ「현금영수증」에서 직접 취소해 주십시오.']
               .join(String.fromCharCode(10));

    const 사유 = await cePrompt(안내 + 돈 + '\n\n취소 사유를 입력해 주십시오.', {
      title: '주문 취소', confirmText: '취소하기', cancelText: '닫기',
  ...
  async function updateOrder(e, opts = {}) {
    ...
    if (wwItems.length > 0) {
      ...
      /* 서버가 「할 일이 없다」고 물러난 갈래를 가려야 한다 — 받아 두지 않아
         아래 잣대가 죽은 코드였다 (2026-09-25 무한테스트 3회차) */
      ww상태   = wwRes.state ?? null;
    }
    ...
    if (!opts.silent) {
      ...
      /* 창고 연계가 실패하면 반드시 창으로 알린다 (2026-09-20 시험).

         여태는 토스트로만 알렸는데, 바로 뒤에 뜨는 「결제 금액 변경」 창이 그것을
         덮었다. 결제는 바뀌고 창고에는 옛 내용이 그대로 선 채로 담당자가 끝난 줄
         알았다 — 두 쪽이 어긋난 것을 아무도 모른다. */
      if (!wwSuccess) {
        ceAlert(
      ...
      /* 창고 줄만은 이 자리에서 고쳐 적는다 (2026-09-25 무한테스트 3회차).

         네 줄(증빙ㆍ결제ㆍ안내ㆍ창고)은 서버가 **첫 걸음**에서 적는다. 그때는 창고에
         아직 아무 말도 하지 않은 때라 그 줄이 늘 「해당 없음」이었다 — 바로 뒤에서
         판매주문을 취소하고 새로 세웠는데도 그렇게 적혔다. 담당자는 창고가 종전
         내용 그대로인 줄 읽는다.

         무엇을 했는지 아는 것은 두 걸음을 다 본 이 자리뿐이다. */
      const 창고줄 = wwItems.length === 0
        ? null
        : (wwSuccess
            ? (ww새번호
                ? `판매주문을 취소하고 새로 등록했습니다 — ${ww새번호}`
                : '판매주문을 다시 맞췄습니다.')
            : `연계하지 못했습니다 — ${wwMessage || '까닭을 받지 못했습니다'}`);

      const 알릴말 = (창고줄 && 돈말)
        ? 돈말.replace(/^창고 — .*$/m, '창고 — ' + 창고줄)
        : 돈말;

      if (알릴말) ceAlert(알릴말, { title: '결제 금액 변경', tone: 'warning' });
    }
  ...
  /* 개인구매는 더 고르지 않지만 이미 담긴 건이 있어 이름은 남긴다(요청서 4쪽).
     교환ㆍ환불이 새로 들어왔다. */
  const _PC_TYPE_MAP   = {'1013':'구매(CE)','1016':'개인구매','1020':'반품',
                          '1021':'교환','1022':'환불','1030':'문의','1050':'기타'};
