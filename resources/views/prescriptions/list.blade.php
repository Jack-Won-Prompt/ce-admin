...
<style>
  ...
  /* 기간(시작 ~ 끝)은 접지 않는다 — 공용 .ds-field-range 는 좁으면 줄을 바꾼다 */
  .rx-date-range { flex-wrap: nowrap; }
...
@section('content')
  ...
  <form method="GET" action="{{ route('prescriptions.index') }}" class="ds-filter-card">
    <div class="ds-filter-fields">
        ...
        <select name="status" class="form-control form-select" onchange="this.form.submit()">
          @php
            ...
            /* 흐름대로 늘어놓는다 — 검수 필요 → 검수 요청 → 검수 완료.
               되물은 건은 그 사이에 둘이 더 선다 (2026-09-15 지시).
                 검수 보류   파일 다시 올리기를 청해 두고 답을 기다리는 건
                 검수 재요청 되물은 자료가 다시 올라와 검수를 청한 건
               「OCR 처리중」은 더 생기지 않아 고르는 자리에서 걷었다.

               목록은 주문이 접수된 건을 감추지만(index), 「주문 완료」를 고르면
               그것만 보인다 — 골라 놓고도 안 보이면 고장 난 것으로 읽힌다. */
            $sts = ['' => '전체', 'review_needed' => '검수 필요', 'review_requested' => '검수 요청',
                    'review_hold' => '검수 보류', 'review_resent' => '검수 재요청',
      ...
      {{-- 기간은 한 줄로 둔다 (2026-09-15 지시) — 두 칸에서는 끝 날짜가 아래로 접혔다. --}}
      <div class="ds-filter-field span-3">
        <label class="ds-field-label">기간</label>
        <div class="ds-field-range rx-date-range">
...
window.HELP_TOUR_STEPS = [
  ...
  },
  {
    selector: '#rxGrid',
    title: '목록 그리드',
    body: '행을 <b>더블클릭</b>하면 주문 화면이 <b>새 탭</b>으로 열립니다(목록 탭은 그대로 유지). <b>판매유형</b>과 <b>위드웍스 판매번호</b> 항목에서 주문 연계 상태를 한눈에 확인할 수 있고, 컬럼 헤더를 클릭해 정렬할 수 있습니다.'
  },
  {
