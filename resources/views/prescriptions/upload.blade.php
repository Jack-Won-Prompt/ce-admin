    ...
    <div class="up-card">
      ...
      <div class="up-card-body">
        <form id="uploadForm" method="POST" action="{{ route('prescriptions.store') }}" enctype="multipart/form-data"
          ...
          {{-- ── 담당자ㆍ메모 (Figma 128:805) — 시안에서 이 카드 안으로 들어왔다 ──

               「처방전 설정」이라 적혀 있었다. 무엇을 설정하는 자리인지 알 수 없다는
               지적을 받았다(2026-09-08 확인요청 8쪽). 이 칸에 담긴 것은 담당자와
               메모뿐이므로 그대로 적는다 — 「처방전 업로드」라 붙이면 바로 위 카드
               제목(처방자료 업로드)과 겹쳐 어느 쪽이 파일을 올리는 자리인지 다시
               헷갈린다. --}}
          <div class="up-sec up-sec-setting">
            <span class="up-sec-title">담당자 · 메모</span>
            <div style="display:flex;flex-direction:column;gap:8px;">
    ...
    <div class="modal-bd">
      <div class="ds-filter-fields">
        ...
        <div class="ds-filter-field span-2">
          ...
          <input type="text" id="pkPhone" class="form-control" placeholder="010-0000-0000" autocomplete="off" data-phone >
        </div>
