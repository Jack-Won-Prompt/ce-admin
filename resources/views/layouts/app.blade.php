...
<script>
  ...
  window.ceWwCols = function (opts = {}) {
    return [
      ...
      {{-- ── 판매현황을 그대로 맞춘 칸 (2026-09-07 지시) ────────────────
           저쪽 목록에 있는데 우리에게 없던 것들. 값이 우리에게 있는 것은 그 자리에서
      ...
      { header: '확정수량',       name: 'ww_conf_qty',  width: 90,  align: 'right', sortable: true },
      { header: '매출 단가',      name: 'ww_unit_price',width: 100, align: 'right' },
      { header: '매출 금액',      name: 'ww_so_amt',    width: 110, align: 'right', sortable: true },
      { header: '제품그룹',       name: 'ww_item_group',width: 110 },
      { header: '피킹유형',       name: 'ww_pick_type', width: 90,  align: 'center' },
      { header: '제품바코드',     name: 'ww_barcode',   width: 130 },
      { header: '등급',           name: 'ww_grade',     width: 80,  align: 'center' },
      { header: '표준코드',       name: 'ww_std_code',  width: 130 },
      { header: 'Description 1',  name: 'ww_desc1',     width: 140 },
      { header: 'Description 2',  name: 'ww_desc2',     width: 140 },
      { header: 'Description 3',  name: 'ww_desc3',     width: 140 },
      { header: 'Description 4',  name: 'ww_desc4',     width: 140 },
      { header: 'S/O Date',       name: 'ww_so_date',   width: 100, align: 'center', sortable: true },
      { header: '라인번호',       name: 'ww_line_no',   width: 90,  align: 'center' },
      { header: '상세 라인번호',  name: 'ww_line_det',  width: 110, align: 'center' },
      { header: '고유 라인 번호', name: 'ww_line_uid',  width: 120, align: 'center' },
      { header: '입고확정일자',   name: 'ww_rcpt_date', width: 110, align: 'center', sortable: true },
      { header: '입고일자',       name: 'ww_in_date',   width: 100, align: 'center', sortable: true },
      { header: '청구서상태',     name: 'ww_inv_state', width: 100, align: 'center' },
      { header: '마감일자 변경여부', name: 'ww_due_chg', width: 120, align: 'center' },
      { header: '발주 번호',      name: 'ww_po_no',     width: 130 },
      { header: '발주일자',       name: 'ww_po_date',   width: 100, align: 'center', sortable: true },
      { header: '배송주소코드',   name: 'ww_addr_code', width: 110 },
      { header: '배송주소',       name: 'ww_address',   width: 240 },
      { header: 'QTY of RB',      name: 'ww_qty_rb',    width: 90,  align: 'right' },
      { header: 'QTY of SB',      name: 'ww_qty_sb',    width: 90,  align: 'right' },
      { header: '영수증 발행상태', name: 'ww_receipt_state', width: 110, align: 'center' },
      { header: '출고번호',       name: 'ww_out_no',    width: 130 },
      { header: '샘플스티커 부착유무', name: 'ww_sticker', width: 130, align: 'center' },
      ...
      {{-- 화면이 제 몫으로 같은 칸을 앞쪽에 두기도 한다(주문 목록ㆍ반품 목록).
           그때는 rrn:false 로 여기서 끈다 — 두면 「주민등록번호」가 두 칸에 서고,
           값이 같아 어느 쪽을 봐야 할지 알 수 없다 (2026-09-16 지시). --}}
      ...(opts.rrn === false ? [] : [
      ...
      { header: '횟수',           name: 'ord_seq',      width: 80,  align: 'right', sortable: true },
      { header: '환급해당기관',   name: 'rx_refund_org',width: 130 },
      { header: '반품 / 샘플 사용목적',   name: 'rt_purpose',      width: 140 },
      { header: '반품 / 샘플 상세목적',   name: 'rt_purpose_note', width: 200 },
  ...
  function showToast(msg, type = 'info', duration = 4000) {
    ...
    /* 앞머리에 박힌 아이콘은 걷어 낸다. 부르는 쪽이 「✅ 저장했습니다」처럼 적어 두면
       여기서 붙이는 아이콘과 겹쳐 「✅✅」가 된다. 아이콘은 갈래가 정하는 것이니
       글에는 없어야 한다 — 부르는 자리를 고쳐 두었지만, 새로 적는 사람이 또 붙일 수
       있어 여기서도 막는다. */
    msg = String(msg).replace(/^\s*[✅❌⚠ℹ⛔🚫✔❗❓🔔💡🎉👍][️︎]?\s*/, '');
    if (msg === '') return;
