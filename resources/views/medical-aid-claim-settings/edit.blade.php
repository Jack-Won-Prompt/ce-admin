...
@section('title', '지급청구서 설정')
@section('page-title', '요양비 지급청구서 설정')
@section('breadcrumb', '홈 - 설정 - 지급청구서 설정')
...
<div class="ds-form fill-rest fill-col">
  ...
  <div class="ds-grid-hint">여기서 정한 자리는 <b>요양비 지급청구서[별지 제12호] PDF</b>에 그대로 반영됩니다.
    기초(의료급여)ㆍ차상위경감 건에 만들어지는 서식입니다.</div>

  <form method="POST" action="{{ route('medical-aid-claim-settings.update') }}" class="fill-rest fill-col">
    @csrf
    @method('PUT')

    <div class="ds-grid-card fill-rest fill-col">
    <div class="pnl-tabs" role="tablist">
      <button type="button" class="pnl-tab active" data-pane="mac-fields"><i class="bx bx-text"></i> 글자 항목 위치</button>
      <button type="button" class="pnl-tab" data-pane="mac-sig"><i class="bx bx-move"></i> 서명 위치</button>
    </div>

    {{-- ── 글자 항목 위치 ──────────────────────────────────── --}}
    <div class="ds-pane active" id="mac-fields">
    <div class="ds-card">
      <h3><i class="bx bx-text"></i> 글자 항목 위치 (원본 PDF 오버레이, 단위 mm)</h3>
      <div class="ds-hint" style="margin-bottom:12px;">
        X는 왼쪽에서, Y는 위에서 잰 거리입니다. 이 서식은 <b>가로 A4(297 × 210)</b>입니다 —
        위임장(세로)과 가로세로가 반대이니 값을 옮겨 적지 마십시오.
        값이 비면 기본 자리를 씁니다. 고친 뒤 <b>지급청구서 PDF</b>를 내려받아 확인하십시오.
      </div>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
          <thead>
            <tr style="background:var(--gray-50);">
              ...
              <th style="text-align:left;padding:8px 10px;border-bottom:1px solid var(--border);font-weight:700;width:90px;">맞춤</th>
            </tr>
          </thead>
          <tbody>
            @foreach($fields as $key => $f)
            <tr>
              <td style="padding:6px 10px;border-bottom:1px solid var(--border-light);">{{ $f['label'] ?? $key }}</td>
              ...
              <td style="padding:6px 10px;border-bottom:1px solid var(--border-light);">
                <input type="number" step="0.5" min="4" max="20" style="width:100%;"
                       name="fields[{{ $key }}][size]" value="{{ old("fields.$key.size", $f['size'] ?? config('medical_aid_claim.size', 8)) }}">
              </td>
              {{-- 맞춤은 코드가 쥔다 — 금액ㆍ개수는 오른쪽 끝을 기준으로 삼아야
                   자릿수가 늘 때 「원」ㆍ「개」 글자를 밀지 않는다 --}}
              <td style="padding:6px 10px;border-bottom:1px solid var(--border-light);color:var(--gray-600);">
                {{ ($f['align'] ?? 'L') === 'R' ? '오른쪽' : '왼쪽' }}
              </td>
            </tr>
            @endforeach
      ...
      <div class="ds-hint" style="margin-top:8px;">
        맞춤이 「오른쪽」인 칸은 X가 <b>오른쪽 끝</b>입니다 — 금액ㆍ개수가 그렇습니다.
      </div>
    </div>
    </div>{{-- /pane --}}

    {{-- ── 서명 위치 ──────────────────────────────────── --}}
    <div class="ds-pane" id="mac-sig">
    <div class="ds-card">
      <h3><i class="bx bx-move"></i> 서명 위치 (단위 mm)</h3>
      <div class="ds-grid">
        <div class="ds-field"><label>X (좌우)</label><input type="number" step="0.1" min="0" max="297" name="sig_x" value="{{ old('sig_x', $setting->sig_x) }}" required></div>
        <div class="ds-field"><label>Y (상하)</label><input type="number" step="0.1" min="0" max="210" name="sig_y" value="{{ old('sig_y', $setting->sig_y) }}" required></div>
        <div class="ds-field"><label>너비 (크기)</label><input type="number" step="0.1" name="sig_w" value="{{ old('sig_w', $setting->sig_w) }}" required></div>
      </div>
      <div class="ds-hint" style="margin-top:8px;">
        기본 자리: X=168, Y=164.5, 너비=20 — 서식의 「(서명 또는 인)」 칸입니다.
        글자 위에 그대로 포개면 서명도 글자도 읽히지 않아, 종이에 서명할 때처럼 살짝 위로 앉힙니다.
      </div>
    </div>
    </div>{{-- /pane --}}
