...
@if($missing)
  <div class="ws-bar warn">
    ...
    주소·토큰이 없으면 주문이 연계되지 않고, 공유 비밀이 없으면 들어오는 콜백을 전부 거절합니다.
  </div>
@endif
...
<form id="wwSaveForm" class="ws-form fill-rest fill-col"
  ...
  <div class="ws-card fill-rest">
    ...
    <div class="ws-bd">
      ...
      @foreach([
        ['cancel_so_type',   '취소', '출고 후 취소일 때만 사용합니다. 출고 전이면 원 판매주문을 취소합니다.', '예: 1504'],
        ['return_so_type',   '반품', '수거해서 창고에 넣습니다.', '예: 1505'],
        ['exchange_so_type', '교환', '회수 입고 후 재출고합니다.', '예: 1506'],
        ['adjust_so_type',   '금액조정', '일반 환불(자격 변경 등)에 사용합니다 — 재고 이동 없이 금액만 마이너스로 정산합니다.', '예: 1092'],
