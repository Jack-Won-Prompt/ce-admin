...
(function () {
  ...
  /** 새 상담을 시작한다 */
  /* 안내 글을 사실에 맞춘다 (2026-09-14 지시).

     여태 「처방전과 주문 줄이 하나씩 함께 섭니다」라고 적어 두었다. 그때는 정말
     그랬으나, 상담 건은 이제 처방전 목록에도 주문 목록에도 서지 않는다 — 통화
     한 번이 곧 한 거래는 아니기 때문이다. 화면이 옛말을 하고 있으면 담당자는
     새 상담을 여는 것을 실제보다 큰일로 여기고 망설인다. */
  window.csNewAsk = async function () {
    const ok = await ceConfirm(
      '이 사람의 새 상담 건을 만듭니다.\n'
      + '상담은 이 상담 이력에만 남습니다 — 처방전 목록과 주문 목록에는 서지 않습니다.\n'
      + '나중에 이 사람의 자료를 올리면 이 상담 건에 이어 붙어 처방전이 됩니다.\n\n'
