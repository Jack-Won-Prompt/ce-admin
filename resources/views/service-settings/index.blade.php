...
@section('help-body')
  ...
  <p>여기서 저장한 항목만 서버 설정을 덮어씁니다. 수정하지 않은 항목은 서버의 <code>.env</code> 값을 그대로 씁니다.</p>
...
<style>
  ...
  /* ── 높이 바닥 잡기 ────────────────────────────────────────────────
     이 화면만 — 얼개의 <main> 은 flex:1 이지만 min-height 가 auto 라, 안의 내용이
     길면 창 높이를 넘겨 제 키대로 늘어난다. 그러면 카드도 함께 늘어나 몸통
     (.ss-body · overflow-y:auto)이 스스로 구를 일이 없어지고, 창이 통째로 구른다.

     1920×1080 에서는 토스페이먼츠 탭이 겨우 들어 눈에 띄지 않았다. 1280×720 에서는
     카드가 1091 로 늘어 창 밖으로 나가고, 아래에 붙여 둔 저장 줄도 함께 밀려났다.

     얼개 맨 바깥(.layout-wrapper)은 min-height:100vh 만 있고 위 한계가 없어, 안이
     길면 거기서부터 늘어난다. 창 높이로 잘라 두면 그 아래 flex 사슬이 차례로
     제 몫만 갖고, 마지막에 몸통이 남는 높이를 받아 스스로 구른다. 중간 마디
     (.layout-page · .content-wrapper · main)의 min-height 가 auto 면 flex 가
     내용보다 작게 줄이지 못하므로 함께 0 으로 푼다.

     이 화면에서만 실리므로 다른 화면은 그대로다 — 목록 화면들은 wwGrid 가 같은
     일을 제 안에서 한다. */
  .layout-wrapper { height: 100vh; }
  .layout-page, .content-wrapper, main { min-height: 0; }
