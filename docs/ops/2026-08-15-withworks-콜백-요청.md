...
`so.shipped` 가 청구의 시작점이다. CEAdmin 은 이 사건을 받는 순간 청구 준비 여부를 다시 따진다.

**`so.delivered` 는 받지 않는다**(2026-08-15 확정). 위드웍스가 배송 상태를 관리하지 않아
— 택배사 조회 연동이 없고 `trackings` 에 배송상태 컬럼도 없다 — 보낼 값이 없다.
그래서 CEAdmin 도 배송완료를 기다리지 않고 출고완료를 기준으로 삼는다.
...
>   있을 테니 그대로 실어 주세요.
>
> **보낼 시점** — 아래 일곱 가지입니다. 상태를 바꾸는 코드 경로를 찾아 각각에 붙여 주세요.
>
> | event | 시점 |
> |---|---|
> | `so.created` | 판매주문 등록 |
...
> | `so.cancelled` | 취소 |
>
> CEAdmin 은 물건이 나가야 공단에 요양비를 청구할 수 있습니다. `so.shipped` 가 안 오면
> 청구가 시작되지 않으니 이 사건은 특히 빠뜨리지 마세요.
> 배송완료(`so.delivered`)는 보내지 않아도 됩니다 — 위드웍스가 배송 상태를 관리하지 않기
> 때문이고, CEAdmin 도 그것을 기다리지 않습니다.
>
> **구현 방식**
...
>
> **확인 방법**
> 테스트 주문 하나로 등록 → 확정 → 할당 → 피킹 → 송장 → 출고를 차례로 진행하고,
