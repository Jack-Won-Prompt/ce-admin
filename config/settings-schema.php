...
return [
    ...
    'popbill' => [
        ...
        'fields' => [
            ...
            'env' => ['label' => '사용 환경', 'config' => 'popbill.env', 'type' => 'select',
                      ...
                      'help'  => '선택한 환경의 계정으로 연결됩니다. 「테스트 모드」는 이 값으로 결정되므로 별도로 설정하지 않습니다. '
                               . '운영으로 설정하면 세금계산서ㆍ현금영수증 발행이 국세청 신고로 처리됩니다.'],
            ...
            'env_sms' => ['label' => '문자 환경', 'config' => 'popbill.service_env.sms', 'type' => 'select', 'width' => 1,
                          ...
                          'help'  => '운영으로 설정하면 문자만 운영 팝빌로 발송되며 실제 요금이 부과됩니다. '
                                   . '수신 대상은 「테스트 설정 › 문자 발송」에서 별도로 지정합니다.'],
            'env_fax' => ['label' => '팩스 환경', 'config' => 'popbill.service_env.fax', 'type' => 'select', 'width' => 1,
                          'options' => ['' => '사용 환경 따름', 'test' => '테스트', 'live' => '운영'],
                          'help'  => '운영으로 설정하면 팩스만 운영 팝빌로 발송되며 실제 출력되고 포인트가 차감됩니다.'],
            'env_taxinvoice' => ['label' => '세금계산서 환경', 'config' => 'popbill.service_env.taxinvoice', 'type' => 'select', 'width' => 1,
                          'options' => ['' => '사용 환경 따름', 'test' => '테스트', 'live' => '운영'],
                          'help'  => '**운영은 국세청 실제 신고로 처리됩니다.** 테스트 기간에는 테스트로 설정하십시오.'],
            ...
            'env_kakao' => ['label' => '알림톡 환경', 'config' => 'popbill.service_env.kakao', 'type' => 'select', 'width' => 1,
                          'options' => ['' => '사용 환경 따름', 'test' => '테스트', 'live' => '운영'],
                          'help'  => '운영으로 설정하면 알림톡만 운영 팝빌로 발송됩니다. 운영 계정에 발신프로필과 템플릿이 등록되어 있어야 합니다.'],
            ...
            'issue_simulate' => ['label' => '발행 시뮬레이션', 'config' => 'popbill.issue_simulate', 'type' => 'bool',
                             'help'  => '켜면 세금계산서·현금영수증의 발행·취소를 팝빌로 전송하지 않고 성공으로 처리합니다. '
                                      . '조회는 팝빌에 그대로 요청합니다. 실제 발행을 확인하려면 해제하되, '
                                      . '「테스트 모드」를 함께 켜서 테스트 사업자번호로 발행되도록 하십시오.'],
            /* 팝빌이 두드릴 자리 (2026-09-18 확인).

               받는 자리는 2026-09-10 에 만들어 두었고 바깥에서 200 으로 응답한다.
               그런데 **팝빌에서 온 것은 한 건도 없다** — 남아 있는 여섯 건은 모두
               우리가 스스로 두드려 본 것이다(사업자번호 1234567890 · 접수번호
               「없는접수번호-fax」). 팝빌 사이트에 이 주소를 등록해야 저쪽이 알려 준다.

               등록 전에는 팩스가 「전송중」에, 세금계산서ㆍ현금영수증이 옛 상태에 멈춰
               있고, 사람이 「대기건 동기화」를 눌러야 맞춰진다.

               주소를 여기 띄우는 까닭은 하나다 — 등록하는 사람이 어디서도 이 값을
               볼 수 없었다. 코드에만 있고 화면에는 없는 값은 등록되지 않는다. */
            'webhook_urls' => [
                'label'  => '전송결과 수신 주소 (팝빌에 등록할 값)',
                'type'   => 'info',
                'source' => 'webhook_urls',
                'width'  => 3,
                'help'   => '팝빌 사이트에 서비스마다 등록해야 전송 결과가 자동으로 수신됩니다. '
                          . '주소에 포함된 긴 문자열이 인증 키이므로 외부에 공개하지 마십시오. '
                          . '등록 전에는 팩스 상태가 「전송중」으로 유지되며, 담당자가 「대기건 동기화」를 실행해야 갱신됩니다.',
            ],
            /* 열쇠를 보고 막을 것인가 (2026-09-18 지시).

               열쇠가 생겼다고 곧바로 막으면, 옛 주소로 오던 것이 그 순간 끊긴다.
               **위의 주소를 먼저 등록하고 나서** 이것을 켠다. 켜면 열쇠 없는 옛 주소는
               401 로 돌려보낸다. */
            'webhook_key_required' => [
                'label' => '인증 키 확인',
                'type'  => 'bool',
                'help'  => '켜면 위 주소로 수신된 것만 처리합니다. **팝빌에 새 주소를 등록한 뒤에** 켜십시오. '
                         . '먼저 켜면 그 사이에 수신되는 전송결과가 누락됩니다.',
            ],
            ...
            /* 팩스 발신번호는 문자와 다르다 (2026-09-18 운영 시험에서 드러남).
               ...
               발신번호(1588-7866)가 그대로 팩스에도 쓰였고, 그 번호는 팩스로 등록된
               적이 없어 보낸 것이 모두 실패했다 — 15건이 그렇게 쌓여 있었다.
               비워 두면 예전처럼 발신 번호를 쓴다. */
            'test_fax_sender'   => ['label' => '팩스 발신번호',   'config' => 'popbill.test.fax_sender',
                             'help'  => '팝빌에 팩스 발신번호로 **등록된** 번호만 쓸 수 있습니다. '
                                      . '문자 발신번호와 다를 수 있으니 팝빌 콘솔의 발신번호 목록과 맞추십시오. '
                                      . '비워 두면 위의 발신 번호를 씁니다.'],

            /* 걷어낸 것 — 문자 시뮬레이션 (2026-09-07 지시).
               문자를 실제로 가르는 것은 「테스트 설정」의 문자 발송(실제ㆍ우리에게만ㆍ
               시뮬레이션)이다. 이 스위치가 보던 sms_simulate 는 그 값이 없을 때만 보는 옛
    ...
    'toss' => [
        ...
        'fields' => [
            ...
            'env' => ['label' => '사용 환경', 'config' => 'toss.env', 'type' => 'select',
                      ...
                      'help'  => '선택한 환경의 키로 연동됩니다. 테스트 모드도 이 값으로 결정됩니다.', 'width' => 3],
            ...
            /* 주소에 박은 열쇠 (2026-09-18 지시).
               ...
               위의 보안 키는 **서명이 붙은 갈래**만 가른다. 정작 우리에게 실제로
               들어오고 있는 가상계좌 입금에는 서명이 없어 아무것도 가르지 못한다.
               열쇠는 서명 없는 갈래까지 덮는다. */
            'webhook_urls' => [
                'label'  => '웹훅 주소 (토스에 등록할 값)',
                'type'   => 'info',
                'source' => 'webhook_urls',
                'width'  => 3,
                'help'   => '토스 개발자센터의 웹훅 주소를 이 값으로 변경하면 인증 키가 함께 적용됩니다. '
                          . '주소에 포함된 긴 문자열이 인증 키이므로 외부에 공개하지 마십시오.',
            ],
            'webhook_key_required' => [
                'label' => '인증 키 확인',
                'type'  => 'bool',
                'help'  => '켜면 위 주소로 수신된 것만 처리합니다. **토스에 새 주소를 등록한 뒤에** 켜십시오. '
                         . '먼저 켜면 그 사이에 수신된 가상계좌 입금이 누락됩니다.',
            ],
    ...
    'order' => [
        ...
        'fields' => [
            'confirm_pay_method' => [
                ...
                'width'   => 3,
                'help'    => '위드웍스에 판매주문이 등록된 뒤 고객에게 이 방식으로 결제를 안내합니다. '
                           . '링크페이는 토스페이먼츠 승인을 받은 뒤에 선택합니다 — 승인 전에는 키가 비어 있어 '
                           . '발송할 수 없으므로 무통장입금으로 대체하여 발송합니다.',
            ],
            'nhis_fax_on_consent' => [
            ...
            'nhis_fax_on_id_card' => [
                ...
                'help'   => '위 설정은 동의가 끝나는 그때만 잽니다 — 그때 신분증이 없으면 걸리고, 뒤에 '
                          . '신분증이 등록되어도 다시 판정하지 않았습니다. 이 항목을 켜면 신분증을 링크로 받거나 첨부로 '
                          . '등록할 때 한 번 더 판정합니다. 이미 발송한 건은 다시 발송하지 않습니다.',
            ],
            ...
            'consent_sms_on_first_save' => [
                ...
                'type'   => 'bool',
                'help'   => '주문 등록에서 처음 저장하여 처방전이 거래처에 연결되는 시점에 서명 SMS 를 함께 발송합니다. '
                          . '이미 동의를 받았거나 유효한 링크가 있으면 발송하지 않습니다. 수동 발송 버튼은 그대로 사용할 수 있습니다.',
            ],
            'consent_sms_hours' => [
                'label'  => '위임동의 발송 시간',
                ...
                'help'   => '이 시간 밖에 저장하면 보내지 않고 화면에 알립니다 — 서명 링크가 30분만 열려, '
                          . '야간에 발송하면 거래처가 확인하는 시점에는 이미 만료됩니다. 예: 09:00-20:00 · 비우면 제한하지 않습니다.',
            ],
            'payment_done_notice' => [
                'label'  => '결제가 끝나면 안내 발송',
                'config' => 'order.payment_done_notice',
                'type'   => 'bool',
                'help'   => '결제가 확인되면 금액과 결제수단을 담아 환자에게 문자를 보냅니다. '
                          . '결제 완료 화면이 「영수증은 문자로 안내드립니다」라고 안내하므로 켜 두는 것이 맞습니다. '
                          . '문구는 메시지 유형 ▸ SMS ▸ 결제 완료 에서 수정합니다. 한 건에 한 번만 발송합니다.',
            ],
            'ship_notice_on_shipped' => [
                ...
                'help'   => '창고가 출고했다고 알려 오면(so.shipped) 운송장 번호를 담아 환자에게 문자를 보냅니다. '
                          . '문구는 메시지 유형 ▸ SMS ▸ 배송 시작 에서 수정합니다. 한 건에 한 번만 발송합니다.',
            ],
        ],
    ],
    ...
    'returns' => [
        ...
        'fields' => [
            ...
            'holidays' => [
                ...
                'type'   => 'textarea',
                'width'  => 3,
                'help'   => 'YYYY-MM-DD 형식으로 쉼표나 줄바꿈으로 구분하여 입력합니다. 토ㆍ일요일은 입력하지 않아도 휴무로 처리됩니다. '
                          . '대체공휴일ㆍ임시공휴일은 매년 관보로 지정되므로 해당 연도 기준으로 확인하여 수정합니다.',
            ],
            /* 자리마다 따로 끄고 켠다 — 문구를 다듬는 동안 한 자리만 멈출 수 있어야 한다 */
            'notice_on_received' => [
                'label'  => '접수하면 안내 발송',
                'config' => 'returns.notice_on_received',
                'type'   => 'bool',
                'help'   => '교환ㆍ반품ㆍ취소를 접수하면 접수번호를 담아 환자에게 문자를 보냅니다. '
                          . '문구는 메시지 유형 ▸ SMS ▸ 교환·반품·취소 접수 에서 수정합니다.',
            ],
            'notice_on_refunded' => [
                'label'  => '환불하면 안내 발송',
                'config' => 'returns.notice_on_refunded',
                'type'   => 'bool',
                'help'   => '환불완료 단계로 옮기면 환불 금액을 담아 환자에게 문자를 보냅니다. '
                          . '교환은 돈이 그대로라 보내지 않습니다. '
                          . '문구는 메시지 유형 ▸ SMS ▸ 환불 처리 완료 에서 수정합니다.',
            ],
            'notice_on_extra_payment' => [
                'label'  => '추가 입금이면 안내 발송',
                'config' => 'returns.notice_on_extra_payment',
                'type'   => 'bool',
                'help'   => '금액조정 방향이 「추가 입금」일 때만 보냅니다. 돌려주는 건은 환불 안내에서 이미 알립니다. '
                          . '문구는 메시지 유형 ▸ SMS ▸ 추가 입금 안내 에서 수정합니다.',
            ],
        ],
    ],
    ...
    'aws' => [
        'label' => 'AWS Textract',
        'desc'  => '처방전 OCR 판독 (설정 › OCR 설정에서 판독기를 선택합니다)',
        'fields' => [
    ...
    'login' => [
        ...
        'fields' => [
            ...
            'password_web' => ['label' => '웹 — 아이디·비밀번호 사용', 'config' => 'auth.password_login.web', 'type' => 'bool',
                               'help'  => '해제하면 관리자 화면에서 아이디ㆍ비밀번호 입력란이 사라지고 Microsoft 계정만 남습니다. ⚠ SSO 가 아직 동작하지 않으므로 해제하면 로그인할 수 없습니다.'],
            'password_app' => ['label' => '앱 — 아이디·비밀번호 사용', 'config' => 'auth.password_login.app', 'type' => 'bool',
                               'help'  => '해제하면 모바일 앱에서 아이디ㆍ비밀번호 입력란이 사라집니다. ⚠ 위와 같은 이유로, 현재 해제하면 앱으로 로그인할 수 없습니다.'],
        ],
    ],
    'mobile' => [
        'label' => '모바일 앱',
    ...
    'web' => [
        'label' => '테스트 설정',
        'desc'  => '테스트 중 외부로 발송되는 항목과 내부에만 머무는 항목을 구분합니다',
        'fields' => [
            'mode' => ['label' => '웹 화면', 'config' => 'web.mode', 'type' => 'select', 'width' => 1,
                       'options' => ['live' => '운영 (번호를 직접 입력)',
                                     'test' => '테스트 (등록된 담당자 번호에서 선택)'],
                       'help'  => '테스트로 설정하면 주문 등록 화면의 휴대폰 입력란이 선택 목록으로 바뀝니다. '
                                . '문자ㆍ알림톡이 실제 거래처로 발송되지 않도록 담당자 번호만 사용합니다.'],

            /* 여태 시뮬레이션이냐 아니냐 둘뿐이라 「정말 나가는지」를 볼 길이 없었다.
            ...
            'fax_mode' => ['label' => '팩스 발송', 'config' => 'popbill.fax_mode', 'type' => 'select', 'width' => 1,
                           'options' => ['live'     => '실제 (관할 지사로)',
                                         ...
                                         'simulate' => '시뮬레이션 (발송하지 않음)'],
                           'help'  => '이 항목이 없을 때는 테스트할 때마다 기준정보의 팩스번호를 변경해 두어야 했습니다. '
                                    . '원복을 누락하면 운영에서 공단으로 팩스가 발송되지 않습니다 — 이 항목으로 대체합니다.'],
            ...
            'withworks_email' => ['label' => '위드웍스 연계 확인 이메일', 'config' => 'web.withworks_email', 'width' => 3,
                                  'help'  => '위드웍스로 주문ㆍ반품을 보낼 때 그 내용을 이 주소로도 보냅니다. '
                                           . '상대 화면과 나란히 놓고 대조할 수 있습니다. 비우면 발송하지 않습니다.'],
