...
return [
    ...
    */
     ...
     * 알 수 없다.
     *
     * 열쇠는 우리 표(App\Support\BillingStrategy)의 것과 같다 — 「유형|자격」. 유형은
     * 10(처방전)과 20(처방외) 둘뿐이다. 원내(30)는 부담 비율도 발행 방식도 원외와 같아
     * 표에서 걷었고, key() 가 10 으로 모은다.
     *
     * 값은 위드웍스의 청구전략 코드다. 아직 어느 코드가 무엇인지 받지 못해 여섯 줄이
     * 모두 25 다 — 코드표를 받으면 이 표만 고치면 되고, 여기 없는 짝은 default 로 나간다.
     * 옆에 적어 둔 것은 우리 표가 정한 부담이다(우리 것이지 저쪽 코드의 뜻이 아니다).
     */
    'withworks_billing_strategy' => [
        'default' => 25,
        'map'     => [
            //  열쇠              코드      우리 표가 정한 부담
            '10|일반'       => 25,   // 본인 10% · 공단 90%
            '10|차상위경감' => 25,   // 공단 100%
            '10|기초'       => 25,   // 지자체(시군구청) 100%
            '10|산재'       => 25,   // 본인 100% (세금계산서는 100%)
            '10|자동차보험' => 25,   // 비율 확인중 — 정해지면 함께 고친다
            '20|'           => 25,   // 처방외 — 본인 100% · 현금영수증 100%
        ],
    ],
