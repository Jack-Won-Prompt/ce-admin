    ...
    private function 처리(Request $request, WithworksSync $sync, ClaimReadiness $readiness): JsonResponse
    {
        ...
        if ($newStatus = self::ORDER_STATUS[$data['event']] ?? null) {
            ...
            /* 기다리고 있던 옛 정정 건을 마저 잇는다.
               ...
               2026-09-20 부터 정정은 취소를 청하는 즉시 새 판매주문을 따로 등록한다
               (OrderAmendService). 그래서 이 길로 오는 건은 **그 전에 걸어 둔
               건**뿐이다 — amend_state 가 requested 인 채로 창고의 되돌림을 기다리던
               주문이다. 그 건까지 버려지면 창고에 아무것도 없는 채로 남으므로
               이 자리는 그대로 둔다.
