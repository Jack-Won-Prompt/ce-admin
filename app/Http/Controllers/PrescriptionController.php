...
"
    ...
    private function consentSmsOnFirstSave(Prescription $prescription): array
    {
        ...
        /* 위임이 필요한 건인지 먼저 본다.
           ...
           이 문자는 「건강보험 급여 위임동의」다 — 우리가 환자를 대신해 공단ㆍ지자체에서
           급여를 받겠다는 위임이다. 산재는 근로복지공단, 자동차보험은 보험사 소관이고
           처방외는 환자가 제 돈으로 산다. **셋 다 우리가 대신 받을 급여가 없다.**
           그런데 자격을 보지 않고 보내고 있었다 — 산재 건에서 만 아홉 살 아이의
           보호자에게 「건강보험 급여 위임동의 서명 요청」이 나갔다
           (2026-09-08 · 3차 5회 송예린).

           자격이 아직 안 정해진 건은 보낸다 — 대개 일반이고, 늦게 보내면 환자를
           두 번 부르게 된다. 정해졌는데 위임이 없는 갈래면 보내지 않는다.

           처방외는 자격을 고를 것도 없이 유형만으로 정해지므로 따로 본다. */
        $유형 = $prescription->counsel_acc_add_type;
        $자격 = $prescription->benefit_class;

        if ($유형 === \App\Support\BillingStrategy::TYPE_NONRX) {
            return $no('처방외는 우리가 청구하지 않아 위임동의를 보내지 않았습니다.');
        }

        if ($자격 !== null && $자격 !== ''
            && ! \App\Support\BillingStrategy::needsDelegation($유형, $자격)) {
            return $no("{$자격}은(는) 급여 위임이 없는 갈래라 보내지 않았습니다.");
        }
