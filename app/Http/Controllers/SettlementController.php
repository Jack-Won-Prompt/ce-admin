...
class SettlementController extends Controller
{
        ...
        return response()->json([
            ...
            'message'      => $반쪽
                ...
                : ($없는돈
                    ...
                    : $order->payMethodLabel() . self::조사_으로($order->payMethodLabel())
                      . ' ' . number_format($due) . '원 입금 확인했습니다.'),
        ]);
    }
     ...
     * 「으로 / 로」를 앞말에 맞춰 고른다.
     *
     * 「가상계좌으로 입금 확인했습니다」로 나갔다 — 결제수단 이름이 여럿이라
     * 문구에 붙박이로 적어 둘 수 없다. 받침으로 가린다(ㄹ 받침은 「로」).
     */
    private static function 조사_으로(string $말): string
    {
        $끝 = mb_substr(trim($말), -1);
        $코드 = mb_ord($끝, 'UTF-8');

        if ($코드 < 0xAC00 || $코드 > 0xD7A3) {
            return '으로';                                  // 한글이 아니면 안전한 쪽으로
        }
    ...
    /**
     * 담당자가 세운 입금 확인을 되돌린다 — 되돌린 것도 기록에 남는다.
     *
