...
class OrderController extends Controller
{
    ...
    public function update(Request $request, Order $order): \Illuminate\Http\JsonResponse
    {
        ...
        /* 정정할 수 없는 주문은 **돈을 건드리기 전에** 막는다 (2026-09-16 고침).

           화면은 ［주문 정정］ 한 번에 요청을 둘 보낸다 — 먼저 이 자리로 금액을
           고치고(아래에서 금액맞추기가 결제 취소ㆍ링크 재발송까지 한다), 그다음에
           위드웍스로 재등록을 보낸다.

           그런데 정정 가능 여부를 뒤쪽(OrderAmendService::정정)에서만 보았다. 이미
           출고된 건이나 정정을 청해 둔 건은 뒤쪽이 422 로 막지만, 그때는 이 자리가
           **이미 결제를 취소하고 새 금액으로 링크를 보낸 뒤**다. 물건은 나갔는데 돈만
           물러난 꼴이 된다.

           화면도 단추를 잠그지만(syncCancelBtns) 열어 둔 사이에 출고가 끝나면 그
           잠금은 옛 값이다. 뒤쪽과 같은 기준으로 여기서 먼저 막는다. */
        if (! $order->정정가능한가()) {
            return response()->json([
                'success' => false,
        ...
        /* 금액이 바뀌었으면 결제도 맞춘다 (2026-09-14 지시 ②ㆍ2026-09-15 고침).

             결제 전  보낸 결제 링크를 해지하고 바뀐 금액으로 다시 보낸다 — 옛
                      금액짜리 링크가 살아 있으면 환자가 그것으로 내고, 그 돈은
                      어디에도 맞지 않는다
             결제 후  받은 돈을 통째로 무르고 바뀐 금액으로 다시 보낸다

           늘든 줄든 한 가지 길이다. 차액만 부분 환불하던 것은 늘어난 때를 담지
    ...
    private static function shippingExtras(Request $request, ?Order $order = null): array
    {
        ...
        /* 출고요청일도 같은 자리에서 담는다 — 창고 전달 메모 바로 위에 있어
           같은 요청으로 온다 (2026-09-11 확인요청 5쪽). */
        foreach (['shipping_postcode', 'shipping_address_detail', 'warehouse_note', 'ship_request_date'] as $col) {
