<?php
// app/Services/Nice/DelegationNiceService.php
...
/**
 * NICE 통합인증 — 운영 데이터 › 위임장 서명 전용 (2026-09-11 지시).
 *
 * 주문 등록의 위임동의가 쓰는 NiceIdentityService 와 **같은 일을 하는 다른 몸**이다.
 * 위임장 서명은 기존 프로세스와 잇지 않기로 정했고(2026-09-11), 그래서 인증 길도
 * 따로 둔다 — 이쪽을 고쳐도 주문 등록이 흔들리지 않고, 그 반대도 그렇다.
 *
 * 자격증명은 함께 쓴다. config('nice.*') 한 곳만 고치면 두 길이 같이 따라온다 —
 * 열쇠를 두 벌 두면 한쪽만 바꿔 두고 왜 안 되는지 찾게 된다.
 *
 * 캐시 열쇠만 갈라 둔다(nice:dlg:). 두 길이 같은 자리를 쓰면 한쪽의 인증이
 * 다른 쪽 것을 지운다.
 *
 * 아래 흐름과 암호 셈은 NiceIdentityService 와 글자 그대로 같다. 손으로 옮겨 적지
 * 않고 복사했다 — 한 글자만 어긋나도 인증이 조용히 깨지기 때문이다. */
class DelegationNiceService
{
     ...
     */
    public function startVerification(DelegationSign $consent, string $returnUrl): array
    {
        $token = $this->token();
     ...
     * 바뀐 것을 몰라도 된다.
     */
    public function handleCallback(DelegationSign $consent, array $params): array
    {
    ...
    private function cacheKey(DelegationSign $consent): string
    {
        return 'nice:dlg:'.$consent->id;
    }
