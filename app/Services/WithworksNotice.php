    ...
    private static function work(string $what): array
    {
        return match ($what) {
            '주문 등록', 'CE샵 주문 등록' => [
                ...
                '작업 수량을 비운 채 바코드부터 입력하면 목표수량이 1 로 설정되어 한 개씩만 피킹됩니다. '
                . '송장번호는 3PL 이 회신한 내용을 「운송장 엑셀업로드」로 등록할 때 반영됩니다.',
            ],

            '주문 수정' => [
                '판매주문 수정',
                ['주문 관리 › 판매 주문에서 변경된 값을 확인해 주십시오'],
                '할당ㆍ피킹이 이미 진행된 건은 수정이 제한됩니다 — 이 경우 취소 후 다시 등록해야 합니다.',
            ],

            '반품 등록' => [
                '반품 입고',
                [
                    '주문 관리 › 판매 반품에서 해당 건을 조회 (상태 「등록」)',
                    '반품 상태를 「확정」으로 변경 후 저장 → 반품 입고가 생성됩니다',
                    '입고 확정 › 입고번호로 검색 › 줄을 더블클릭',
                    '상세 표의 「비고」에 검수 내용을 입력 → 「확정」',
                ],
                '상세 줄에 입력한 비고는 CE Admin 의 「창고 검수 비고」로 그대로 전달됩니다 — '
                . '검수 내용이 CE 담당자의 판단 근거가 됩니다.',
            ],

            '반품 상태 전달' => [
                '확인만',
                ['주문 관리 › 판매 반품에서 적요의 「[CE 상태]」 줄을 확인'],
                'CE Admin 의 진행 상태를 안내하는 것이므로 별도 조치는 필요하지 않습니다. '
                . '다만 CE 에서 「취소」로 전송하면 아직 처리하지 않은 반품만 취소됩니다.',
            ],

            '금액조정 주문 등록' => [
                '금액조정 (출고 없음)',
                [
                    '주문 관리 › 판매 주문에서 해당 건을 조회 (판매유형 「전산판매(금액조정)」)',
                    '출고하지 않습니다 — 금액 정산용 주문입니다',
                ],
                '부분 반품ㆍ자격 변경과 같이 반품 후에도 정산 차액이 남는 건에서 생성됩니다.',
            ],

            default => ['확인', ['주문 관리에서 이 건을 확인하십시오'], ''],
        };
    }
