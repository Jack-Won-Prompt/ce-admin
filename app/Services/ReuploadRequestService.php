...
class ReuploadRequestService
{
    ...
    public function 걸기(
        ...
        /* 되물은 건은 「검수 보류」로 세운다 (2026-09-15 지시).

           여태 review_needed 로 되돌렸는데, 그러면 「아직 아무도 안 본 건」과
           섞여 검수자가 무엇을 지금 볼지 가릴 수 없었다. 보류는 답을 기다리는
           자리다 — 목록에서 따로 서야 한다.

           이미 마친 건이라도 되돌린다 — 되물을 자료가 있다는 것은 아직 볼 것이
           남았다는 뜻이다. 다만 **주문이 이미 접수된 건은 상태를 내리지 않는다**.
           창고로 넘어간 주문의 처방전이 보류로 내려가면, 주문 목록과 처방전
           목록이 서로 다른 것을 말하게 된다 — 그 건은 요청 자취만 남긴다. */
        if ($처방전->status !== 'ordered' && $처방전->status !== 'review_hold') {
            $처방전->update(['status' => 'review_hold']);
        }
