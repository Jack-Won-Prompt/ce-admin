...
class OrderCancelService
{
    ...
    public function 취소(Order $order, string $사유): array
    {
        ...
        if ($단계 === 'new') {
            ...
            $창고말 = '창고에 취소를 요청했습니다 — 할당ㆍ피킹이 해제되면 자동으로 취소됩니다.';
        }
    ...
    public function 정정미리보기(Order $order, int $바뀔금액, int $바뀔기관): string
    {
        ...
        if ($바뀔금액 === $이전 && ! $기관바뀜) {
            return self::정정안내틀([
                '창고' => $order->withworks_so_no
                            ? "판매주문 {$order->withworks_so_no} 취소 후 재등록합니다."
                            : '창고로 전달하지 않은 건입니다.',
            ]);
        }
        ...
        $결제 = match (true) {
            ...
            ! $order->isDepositConfirmed() => '입금 전이므로 환불할 금액이 없습니다.',
            /* 입금 확인은 서 있는데 받은 돈이 0으로 읽히는 건 — 결제 기록이 어긋난
               것이다. 「0원을 환불합니다」라 적으면 아무 뜻도 없다. */
            $이전 <= 0 => '결제 기록이 0원으로 남아 있어 환불할 금액이 없습니다 — '
                        . '결제 내역을 확인해 주십시오.',
            (bool) $order->tossPayment?->is_done
                => '기결제액 ' . number_format($이전) . '원을 환불합니다.',
            default
                => '담당자 확인 입금액 ' . number_format($이전) . '원입니다 — '
                 . 'PG 결제 내역이 없어 자동 환불되지 않습니다. 계좌이체로 환불해 주십시오.',
        };
    ...
    public function 금액맞추기(Order $order, int $이전): string
    {
        ...
        /* 받은 돈이 0으로 읽히는 건 — 무를 것이 없다 (2026-09-23 무한 테스트).

           입금 확인은 서 있는데 결제기준금액이 0이면 결제 기록이 어긋난 것이다 —
           앞 결제의 취소액이 남아 「받은 돈 − 취소액」이 0이 되는 따위다
           (PaymentLinkController::record 에서 고쳤다). 그대로 두면 토스에 0원 취소를
           걸고 「기결제액 0원을 환불 처리했습니다」라 적는데, 실제로 돌아간 돈은
           없다. 무엇이 어긋났는지 적어 사람이 보게 한다. */
        if ($이전 <= 0) {
            $order->입금확인취소(sprintf(
                '주문 정정 — 결제 기록이 0원이라 환불하지 않음, %s원으로 재청구', number_format($지금)
            ));

            return self::정정안내틀([
                '증빙' => $증빙말,
                '결제' => '결제 기록이 0원으로 남아 있어 환불하지 않았습니다 — '
                        . '결제 내역을 확인해 주십시오.',
        ...
        if (! $order->tossPayment?->is_done) {
            ...
            return self::정정안내틀([
                '증빙' => $증빙말,
                '결제' => '담당자 확인 입금액 ' . number_format($돌려줄돈) . '원입니다 — '
