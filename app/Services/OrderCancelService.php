...
class OrderCancelService
{
     ...
     *                  (2026-09-15 고침).
     */
    public function 금액맞추기(Order $order, int $이전, ?int $이전기관 = null): string
    {
        ...
        $지금기관 = (int) ($order->insurance_amount ?? 0);
        $기관바뀜 = $이전기관 !== null && $이전기관 !== $지금기관;
        ...
        if ($지금 === $이전) {
            return $this->증빙무르기($order, $이전기관, $지금기관);
        }
