...
class PhoneNo
{
    ...
    public static function format(?string $raw): string
    {
        ...
        /* 050X 로 시작하는 안심번호ㆍ인터넷팩스는 앞이 네 자리다. 이것만은 붙임표가
           있어도 다시 나눈다 — 세 자리로 끊어 쌓인 옛 값(050-4134-1393)을 화면에서는
           바로잡아 보여 주기 위해서다. 저장된 값은 그대로 둔다. */
        if (preg_match('/^050\d/', $d)) {
            return match (strlen($d)) {
                12      => substr($d, 0, 4) . '-' . substr($d, 4, 4) . '-' . substr($d, 8),
                11      => substr($d, 0, 4) . '-' . substr($d, 4, 3) . '-' . substr($d, 7),
                default => $raw,
            };
        }
