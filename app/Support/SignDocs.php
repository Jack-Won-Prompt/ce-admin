...
final class SignDocs
{
    ...
    public static function 목록(PrescriptionConsent $consent, bool $privacyDone = false): array
    {
        ...
        /* 요양비 지급청구서 — 기초(의료급여)ㆍ차상위경감 (2026-09-17 지시).
           처음에는 기초만 보았는데(2026-09-01 회신), 차상위경감도 본인부담이 없고
           기관이 전액을 내므로 같은 청구서를 낸다. */
        $자격 = trim((string) ($rx?->benefit_class ?? ''));
        if (in_array($자격, \App\Support\MedicalAidClaimForm::자격, true)) {
            $목록[] = self::줄(self::청구서, 'pdf',
                $자격 === '기초'
                    ? '얼마를 청구하는지 적어 시군구청에 내는 서류입니다.'
                    : '얼마를 청구하는지 적어 공단에 내는 서류입니다.');
        }

        /* 등록신청서는 여기 세우지 않는다 (2026-09-21 지시).

           공단에 내는 등록신청서는 **병원이 ② 요양기관 확인란을 적어 내준 종이**다.
           그것을 찍어 올리고, 비어 있는 ③ 신청인란에 서명과 이름을 얹는 것이
           우리가 하는 일이다 — RegistrationOverlay 가 그 일을 한다.

           여태는 빈 서식(별지 제4호)을 우리가 새로 그려 한 장 더 만들었다. 그러면
           한 건에 「등록신청서」가 둘이 서고, 그 가운데 우리가 그린 쪽은 병원 확인란이
           비어 있어 **그대로는 공단에 낼 수 없는 장**이었다. 담당자는 목록에서 같은
           이름 둘을 보고 어느 것을 내야 하는지 가려야 했다.

           서명은 올려 둔 그 장에 얹히므로, 환자가 서명 화면에서 볼 것도 없다. */
