...
final class SignDocs
{
    ...
    public static function 목록(PrescriptionConsent $consent, bool $privacyDone = false): array
    {
        ...
        /* 등록 신청서 — **공단에 등록하는 건**의 등록ㆍ재등록 진행 중인 건만.

           서식이 「요양비의 보험급여 기준 및 방법 [별지 제4호서식]」이라 맨 아래가
           「국민건강보험공단 이사장 귀하」다. 공단에 내는 종이다.

           위임이 필요 없는 갈래(처방외ㆍ산재ㆍ자동차보험)는 공단에 낼 일이 없고
           (2026-09-11 바로잡음), 기초(의료급여)도 공단이 아니라 시군구청에 낸다 —
           그쪽은 등록신청서 없이 요양비 지급청구서만 낸다(2026-09-21 확인). 여태
           위임 판정만 보아 기초 건에도 공단 앞 서식을 내밀고 있었다. */
        $공단건 = ($rx?->claim_agency ?: ClaimAgency::fromBenefitClass($rx?->benefit_class))
                === ClaimAgency::NHIS;
