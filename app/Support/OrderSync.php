...
/**
 * 유형을 고른 건은 주문 관리에도 선다.
 *
 * 처방전 그림이 없어도, 제품을 아직 안 골랐어도 그렇다 — 유형을 고른 건은 담당자가
 * 「이 건을 주문으로 진행한다」고 밝힌 것이고, 그것을 보는 자리가 주문 관리다.
 * 예전에는 「주문 생성 및 연계」를 눌러야만 줄이 생겨, 손대던 건이 어느 목록에도 없이
 * 떠 있었다.
 *
 * **저장만으로는 서지 않는다** (2026-09-22 확인요청 2ㆍ4쪽). 한동안은 담기기만 하면
 * 세웠는데, 그러면 검수도 안 끝난 건과 창고로 넘기지도 못한 건이 주문 관리에 줄로
 * 남았다. 잣대는 seed() 안에 적어 두었다.
 *
...
class OrderSync
{
    ...
    public static function seed(Prescription $prescription): ?Order
    {
        ...
        /* **유형을 고른 건만 세운다** (2026-09-22 확인요청 2ㆍ4쪽).

           여태는 처방전이 담기기만 하면 줄이 섰다. 그래서 —

             · 처방전 목록에서 「검수 필요」인 건이 주문 등록ㆍ주문 관리에 라인으로
               서 있었다(확인요청 2쪽). 운영에서 검수가 안 끝난 처방에 달린 주문이
               스물한 건이었다(2026-09-22 서버 확인).
             · 정정할 수 없어 창고로 넘기지도 못한 건이 주문 관리에 줄로 남았다
               (확인요청 4쪽). 판매번호 없는 주문이 서른한 건이다.

           주문번호는 위드웍스ㆍ토스ㆍ팝빌ㆍ공단으로 나가는 대외 식별자다. 저장 한 번에
           저절로 태울 번호가 아니라, **담당자가 「이 건을 주문으로 진행한다」고 밝혔을
           때** 나야 한다. 그 뜻을 밝히는 걸음이 유형(처방전ㆍ처방외)을 고르는 것이다 —
           그래서 유형을 고르는 자리에서 곧바로 줄을 세우고
           (PrescriptionController::createOrderLine), 여기서는 그 값이 있는지만 본다.

           이미 선 줄은 건드리지 않는다. 지난 건이 이 잣대로 사라지지 않는다. */
        if ((string) ($prescription->counsel_acc_add_type ?? '') === '') {
            return null;
        }
