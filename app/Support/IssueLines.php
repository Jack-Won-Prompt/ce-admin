...
final class IssueLines
{
     ...
     * 소비자가 1,500 → 단가 1,364. 전액 발행 건은 수량 × 단가가 공급가액과 맞고,
     * 기관 몫만 발행하는 건(일반 90%)은 맞지 않는다 — 수량을 지키기 때문이다.
     *
     * 예전에는 줄이 하나면 수량을 그대로 두고 단가 자리에 공급가를 통째로 적었다 —
     * 「수량 180 · 단가 331,364」처럼 곱이 크게 어긋난 줄이 신고되었다. 그것을 고치려고
        ...
        foreach ($rows as $i => $r) {
            ...
            $out[] = [
                ...
                /* 단가는 **소비자가에서 부가세를 구분한 값**이다 (2026-09-26 지시).
                   ...
                   소비자가 1,500 은 부가세가 든 값이고, 세금계산서의 단가ㆍ공급가액은
                   부가세를 뺀 값이다. 그래서 1,500 ÷ 1.1 = 1,364 를 적는다.

                   여태 그 줄의 공급가를 수량으로 나눈 값을 적었다. 그러면 90%만
                   발행하는 건에서 1,227 이 되어 실제 판 값과 이어지지 않았다.

                   전액을 발행하는 전략(차상위경감ㆍ기초 100%)은 수량 × 단가가
                   공급가액과 맞는다. 기관 몫만 발행하는 전략(일반 90%)은 맞지 않는데,
                   150개를 팔고 그중 90% 값어치만 청구하기 때문이다 — 수량을 135로
                   줄이면 실제 보낸 수량과 서류가 달라지므로 수량 쪽을 지킨다. */
                'unit'   => isset($r['unit'])
                    ? (int) round($r['unit'] / (1 + self::VAT_RATE))
                    : (int) round($s / $qty),
                'supply' => $s,
                'vat'    => $t,
                'amount' => $r['amount'],
