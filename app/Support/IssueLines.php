...
final class IssueLines
{
     ...
     *
     * 단가는 **소비자가**다 (2026-09-26 지시). 판 값을 그대로 보이는 것이 먼저라,
     * 기관 몫만 발행하는 건에서는 수량 × 단가가 공급가액과 맞지 않는다.
     *
    ...
    public static function split(Order $order, int $supply, int $vat): array
    {
        ...
        foreach ($rows as $i => $r) {
            ...
            $out[] = [
                ...
                'device' => $r['device'],
                'qty'    => $qty,
                /* 단가는 **소비자가**를 적는다 (2026-09-26 지시).

                   여태 그 줄의 공급가를 수량으로 나눈 값을 적었다. 그러면 90%만
                   발행하는 건에서 소비자가 1,500 인 제품이 계산서에 1,227 로 보인다 —
                   받는 쪽이 실제 판 값을 알 수 없다.

                   **이 바꿈으로 수량 × 단가 ≠ 공급가액이 되는 건이 있다.** 기관 몫만
                   발행하는 전략(일반 90%)이 그렇다. 전액을 발행하는 전략(차상위경감ㆍ
                   기초 100%)은 부가세를 뺀 값이라 여전히 다르다 — 소비자가는 부가세가
                   든 값이기 때문이다. */
                'unit'   => (int) ($r['unit'] ?? round($s / $qty)),
                'supply' => $s,
                'vat'    => $t,
                'amount' => $r['amount'],
            ];
