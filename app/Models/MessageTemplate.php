...
class MessageTemplate extends Model
{
    ...
    public static function defaultSms(): array
    {
        return [
            ...
            /* 결제 안내 문자 — 본문이 코드에 박혀 있던 것을 여기로 옮겼다
               (2026-09-25 무한테스트). 발송 이력의 유형 칸도 이제 채워진다. */
            \App\Services\PaymentLinkService::유형_링크 => [
                'label' => '결제 링크 안내',
                'desc'  => '링크페이로 받을 때 — 결제전송에서 자동 발송',
                'text'  => '[#{회사}] #{고객명}님, #{제품} 결제 안내입니다.' . chr(10)
                         . '#{취소줄}금액: #{금액}원' . chr(10)
                         . '아래 링크에서 결제해 주시기 바랍니다.' . chr(10)
                         . '#{링크}' . chr(10)
                         . '링크는 #{유효일}일간 유효합니다.',
            ],
            \App\Services\PaymentLinkService::유형_계좌 => [
                'label' => '무통장 입금 안내',
                'desc'  => '우리 계좌로 받을 때 — 결제전송에서 자동 발송',
                'text'  => '[#{회사}] #{고객명}님, #{제품} 결제 안내입니다.' . chr(10)
                         . '#{취소줄}금액: #{금액}원' . chr(10)
                         . '입금: #{입금처}' . chr(10)
                         . '입금자명은 주문자 성함과 동일하게 기재해 주시기 바랍니다.',
            ],
            \App\Services\PaymentLinkService::유형_가상계좌 => [
                'label' => '가상계좌 안내',
                'desc'  => '토스에서 가상계좌를 골랐을 때 — 승인 직후 자동 발송',
                'text'  => '[#{회사}] #{고객명}님, 입금하실 계좌입니다.' . chr(10)
                         . '#{계좌}' . chr(10)
                         . '#{예금주줄}금액 #{금액}원#{기한줄}',
            ],
