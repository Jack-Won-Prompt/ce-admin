...
class Patient extends Model
{
    ...
    protected static function booted(): void
    {
        ...
        /* 공단 위임 등록일이 바뀌면 청구 준비를 다시 따진다 (2026-09-18 지시).

           위임 등록이 없으면 공단이 청구를 받지 않는다. 그런데 등록일은 거래처
           화면에서 들어오고 주문은 건드리지 않아, 등록을 받아 적어도 주문 목록은
           「공단 위임 등록 없음」인 채로 남았다. 여태 한 시간마다 훑어 메웠다. */
        static::saved(function (self $p) {
            if ($p->wasChanged('nhis_agree_start')) {
                app(\App\Services\ClaimReadiness::class)->환자다시보기($p->id);
            }
        });
    ...
    {
        /* 가려진 값이 되돌아오면 적어 둔 것을 그대로 둔다 (2026-09-17 시험에서 드러남).

           화면은 주민번호를 「120315-3******」처럼 가려 보여 준다. 담당자가 그 칸을
           건드리지 않고 저장하면 그 가려진 글이 그대로 돌아오는데, 여태 그것을
           번호로 알고 암호문을 다시 만들었다 — 적어 둔 번호가 별표로 덮여
           「저장했는데도 없다」가 되었고, 세금계산서가 「개인 발행에는 주민등록번호
           13자리가 필요합니다」로 계속 막혔다.

           별표가 섞인 값은 「고치지 않았다」는 뜻이다. 그대로 두고 지나간다. */
